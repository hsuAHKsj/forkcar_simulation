...
namespace PurePursuitNS
{
  PurePursuit::PurePursuit() : is_global_path_sub_(false), is_current_pose_sub_(false) 
  {
    ...
    odom_sub_ = nh_.subscribe("/forklift_drive_controller/odom", 10, &PurePursuit::odomCallback, this);
  ...
  void PurePursuit::run()
  {
    ...
    if((alpha >= 5.0 * M_PI / 180.0 || alpha <= -5.0 * M_PI / 180.0) && obstacle_num_ == 0 && path_range <= 0.5)//纠正线路
    {
